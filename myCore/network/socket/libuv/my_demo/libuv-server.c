...
void echo_read(uv_stream_t *client, ssize_t nread, const uv_buf_t *buf) {
    ...
    if (nread > 0) {
        printf("[%s]Server received %ld bytes\n",timestamp, nread);
		...
		memset(req->buf.base,'S',nread);
        uv_write((uv_write_t*) req, client, &req->buf, 1, echo_write);
        printf("[%s]Server send %ld bytes\n",timestamp, nread);
        // Send an additional response
        /*write_req_t *resp = (write_req_t*) malloc(sizeof(write_req_t));
        resp->buf.base = strdup("Server received your data\n");
        resp->buf.len = 26;
		*/
		//memset(req->buf.base,'S',nread);
        //uv_write((uv_write_t*) req, client, &req->buf, 1, echo_write);
    ...
    // Always free the buffer, regardless of nread
    if (buf->base)
        free(buf->base);
}
